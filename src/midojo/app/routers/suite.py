...
def suite_info(suite: Annotated[YAMLTaskSuite, Depends(get_suite)]):
    return SuiteInfoResponse(
        ...
        injection_vectors=suite.get_injection_vector_names(),
        environment=suite.load_and_inject_default_environment({}),
    )


@router.get("/injection-vectors", status_code=status.HTTP_200_OK)
def injection_vectors(suite: Annotated[YAMLTaskSuite, Depends(get_suite)]) -> list[str]:
    return suite.get_injection_vector_names()
