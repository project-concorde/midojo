"""Verifiers — the second of the engine's two observability axes.

A *verifier* defines *how an outcome is checked*. This module is the
provider-agnostic framework: the :class:`Verifier` protocol, the
:class:`VerificationContext` a verifier inspects, the parsed :class:`Check`, and
the registry that dispatches a check block to the verifier that owns it. It
knows nothing about any concrete verifier.

Concrete verifiers live in :mod:`midojo.verifiers` — the built-in predicate
checks (:mod:`midojo.verifiers.builtin`), and later filesystem inspection,
network-traffic analysis, Red Hat ACS / StackRox runtime checks (e.g. "did a
new process spawn?", "was there a network request to domain X?").
